...
psi = 1;             % Disutility of labor scaling
...
%% Value Function Iteration with Endogenous Labor Supply
...
policy_l = zeros(N, grid_size);  % Policy function for labor supply
...
for iter = 1:max_iter
    ...
    for i = 1:N
        for j = 1:grid_size
            % Labor supply decision (first-order condition)
            % labor_l function calculates optimal labor given wages and consumption
            l_guess = ((w * s(i)) / psi)^(1 / (eta + 1));  % Initial guess for labor
            % Budget constraint with endogenous labor
            c = (1 + r) * a_grid(j) + w * s(i) * l_guess - a_grid;

            c(c <= 0) = NaN;
            if sigma == 1
               u = log(c) - (psi * (l_guess^(1 + eta)) / (1 + eta)); 
            else
               u = (c.^(1 - sigma) - 1) / (1 - sigma) - (psi * (l_guess^(1 + eta)) / (1 + eta));
            end

            [v_new(i, j), idx] = max(u + beta * P(i, :) * v_old);
            policy_k(i, j) = a_grid(idx);
            policy_l(i, j) = l_guess;  % Store optimal labor supply
            policy_c(i, j) = (1 + r) * a_grid(j) + w * s(i) * policy_l(i, j) - policy_k(i, j);
        end
    end
