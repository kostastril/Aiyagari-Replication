...
phi = 1;             % Weight on labor disutility
theta = 1;           % Labor elasticity
b = 0;               % Debt limit
...
%% EGM Method with Endogenous Labor Supply
...
% Utility functions
u_prime_c = @(c) c.^(-sigma);
u_prime_c_inv = @(u_p) u_p.^(-1 / sigma);
u_prime_l = @(l) phi * l.^theta;
u_prime_l_inv = @(u_l) (u_l / phi).^(1 / theta);
...
while dist > tol && iter < max_iter
    ...
    policy_k = zeros(size(policy_c));
    policy_l = zeros(size(policy_c));
 
    % Step 1: Compute RHS of Euler equation
    RHS = zeros(grid_size, N);
    for j = 1:N
        for m = 1:N
            RHS(:, j) = RHS(:, j) + beta * (1 + r) * P(j, m) * u_prime_c(policy_c(:, m));
        end
    end

    % Step 2: Invert marginal utility to find consumption
    c_next = u_prime_c_inv(RHS);

    % Step 3: Compute endogenous grid and interpolate policies
    for j = 1:N
        labor_supply = u_prime_l_inv((w * s(j) .* u_prime_c(c_next(:, j))));
        a_hat = (c_next(:, j) + a_grid - w * s(j) * labor_supply) / (1 + r);

        % Interpolate consumption policy and apply borrowing constraint
        g_c_temp = interp1(a_hat, c_next(:, j), a_grid, 'linear', 'extrap');
        g_c_temp(a_grid < amin) = amin; % Apply borrowing constraint
        policy_c_next(:, j) = g_c_temp;

        % Update labor policy based on optimal consumption
        policy_l(:, j) = u_prime_l_inv((w * s(j) .* u_prime_c(policy_c_next(:, j))));

        % Update asset policy based on budget constraint
        policy_k(:, j) = (1 + r) * a_grid + w * s(j) * policy_l(:, j) - policy_c_next(:, j);
        policy_k(policy_k < 0 ) = 0;
    end
