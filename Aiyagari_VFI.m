...
%% Compute Wealth Shares for Quintiles

% Sort wealth in ascending order
sorted_k = sort(sim_k);

% Number of households (or agents)
n = length(sorted_k);

% Determine the indices for the quintiles
q1_idx = round(n * 0.2);  % 20th percentile
q2_idx = round(n * 0.4);  % 40th percentile
q3_idx = round(n * 0.6);  % 60th percentile
q4_idx = round(n * 0.8);  % 80th percentile

% Compute total wealth in the economy
total_wealth = sum(sorted_k);

% Calculate wealth held by each quintile
wealth_q1 = sum(sorted_k(1:q1_idx));  % Bottom 20%
wealth_q2 = sum(sorted_k(q1_idx+1:q2_idx));  % Next 20% (20-40%)
wealth_q3 = sum(sorted_k(q2_idx+1:q3_idx));  % Next 20% (40-60%)
wealth_q4 = sum(sorted_k(q3_idx+1:q4_idx));  % Next 20% (60-80%)
wealth_q5 = sum(sorted_k(q4_idx+1:end));  % Top 20%

% Compute the share of total wealth held by each quintile
share_q1 = (wealth_q1 / total_wealth) * 100;
share_q2 = (wealth_q2 / total_wealth) * 100;
share_q3 = (wealth_q3 / total_wealth) * 100;
share_q4 = (wealth_q4 / total_wealth) * 100;
share_q5 = (wealth_q5 / total_wealth) * 100;

% Display the wealth shares for each quintile
disp(['Wealth share of bottom 20%: ', num2str(share_q1), '%']);
disp(['Wealth share of next 20%: ', num2str(share_q2), '%']);
disp(['Wealth share of next 20%: ', num2str(share_q3), '%']);
disp(['Wealth share of next 20%: ', num2str(share_q4), '%']);
disp(['Wealth share of top 20%: ', num2str(share_q5), '%']);

%% Plot Wealth Shares for Quintiles

figure;
bar([share_q1, share_q2, share_q3, share_q4, share_q5], 'FaceColor', 'b');
title('Wealth Distribution Across Quintiles');
xlabel('Quintiles');
ylabel('Wealth Share (%)');
xticklabels({'Bottom 20%', 'Next 20%', 'Next 20%', 'Next 20%', 'Top 20%'});
grid on;
